  ...
  if File.exist?('person.json')
    ...
    people_data.each do |person|
      if person["specialization"]
        ...
        people.push(Student.new(person["age"], name: person["name"], parent_permission: person['parent_permission'], id: person['id']))
      end
    end
  end
 people
...
def read_rentals(books, people)
  ...
  if File.exist?('rentals.json')
    ...
    rentals_data.each do |rental|
      ...
      person = people.select { |person| person.id == rental['person_id'] }
